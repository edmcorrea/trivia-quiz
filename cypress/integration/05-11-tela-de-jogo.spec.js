...
const { fetch: fetchMock, mockFetchWithToken} = require('../mocks/fetch');
const { questionsResponse } = require('../mocks/questions');
const { tokenResponse } = require('../mocks/token');

const FETCH_TRIVIA_URL = `https://opentdb.com/api.php?amount=5&token=${tokenResponse.token}`;
...
describe('10 - [TELA DE JOGO] Crie um botão de `Next` que apareça após a resposta ser dada', () => {
  ...
  it('Será validado se o botão "Next" não é visível no início do jogo', () => {
    cy.get(BUTTON_NEXT_QUESTION_SELECTOR).should('not.exist');
  });
