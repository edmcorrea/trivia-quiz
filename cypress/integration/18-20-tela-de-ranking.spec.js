...
describe.only('19 - [TELA DE RANKING] Crie o conteúdo da tela de _ranking_', () => {
  beforeEach(() => {
    cy.visit('http://localhost:3000/', {
      onBeforeLoad(win) {
        win.fetch = fetchMock;
...
describe('20 - [TELA DE RANKING] Desenvolva testes para atingir 90% de cobertura da tela de Rankings', () => {
  it('Verifica a cobertura de testes unitários', () => {
    cy.task('getCoverage', getId()).its('Ranking.functions.pct', { timeout: 0 }).should('be.gte', 90.00);
    cy.task('getCoverage', getId()).its('Ranking.lines.pct', { timeout: 0 }).should('be.gte', 90.00);
  });
});
